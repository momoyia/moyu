...
def create_backup(folders):
    ...
    backup_dir = f"assets_stay_food_backup_{timestamp}"
    ...
    for folder in folders:
        if os.path.exists(folder):
            ...
            print(f"   ✅ 备份 {folder} → {backup_folder}")
...
def main():
    """主函数"""
    print("🖼️  开始压缩 stay 和 food 文件夹的图片...")
    print(f"📋 压缩配置:")
    ...
    # 创建备份
    backup_dir = create_backup(existing_folders)
    print()
    ...
    for folder in existing_folders:
        ...
        for image_file in image_files:
            print(f"🔄 压缩: {os.path.basename(image_file)}")
    ...
    print(f"   - 如需恢复: cp -r {backup_dir}/* assets/images/")
